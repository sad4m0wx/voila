<!-- Meeting Point Display - Conservative replacement for MeetingPointResults -->
<script>
  ...
  $: canNavigatePrev = hasMultipleMeetingPoints;
  $: canNavigateNext = hasMultipleMeetingPoints;
  $: isMobile = variant === 'card';
  $: canCalculate = addresses.length >= 2 && addresses.every(addr => addr.value.trim());
  ...
  // Set initial state for card variant
  $: if (variant === 'card' && !meetingPoint && showAddressForm === false) {
    showAddressForm = true;
  }
...
{#if isCalculating}
  <!-- Loading State -->
  <div class="text-center py-6 {variant === 'card' ? 'mobile-card-elevated p-4 mb-4' : 'card card-gradient p-6 mb-4'}">
    <LoadingIndicator 
      size={variant === 'card' ? 'md' : 'lg'} 
      variant="native" 
      text={variant === 'card' ? `Finding your perfect spot...` : `Calculating optimal meeting point...`}
    />
    {#if variant === 'card' && addresses.length > 0}
      <p class="text-sm text-gray-500 mt-2">Analyzing {addresses.length} locations</p>
    {/if}
  </div>

{:else if meetingPoint && (!showAddressForm || variant !== 'card')}
  <!-- Results Display -->
  <div class="relative {variant === 'card' ? 'mobile-card-elevated p-4 mb-4' : ''}">
    ...
    >
      <div class="{variant === 'card' ? '' : 'card card-gradient p-4 mb-4'} animate-fade-in">
        
        <!-- Card Variant Header (Mobile) -->
        {#if variant === 'card'}
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center">
              ...
              <div>
                <h3 class="font-bold text-gray-900">Meeting Point Found!</h3>
                <p class="text-sm text-gray-500">{meetingPoint.name || 'Perfect location'}</p>
              </div>
            </div>
            <button
                ...
                <div class="flex items-center justify-between p-3 rounded-lg border border-neutral-200 bg-white/50 hover:bg-white/80 transition-colors">
                  <div class="flex items-center flex-1 min-w-0">
                    <div class="w-6 h-6 rounded-full bg-primary-100 text-primary-700 flex items-center justify-center text-xs font-semibold mr-2 flex-shrink-0">
                      {variant === 'card' && addresses.length > 0 ? addresses.findIndex(addr => addr.id === time.id) + 1 : index + 1}
                    </div>
                    <div class="min-w-0 flex-1 overflow-hidden">
                      <p class="font-medium text-secondary-800 truncate text-xs {variant === 'card' ? 'max-w-32' : 'max-w-[120px] md:max-w-[180px]'}">
                        {time.address}
                      </p>
                    </div>
                  </div>
...
{:else if variant === 'card'}
  <!-- Address Input Mode for Card Variant -->
  <div class="mobile-card-elevated p-4 mb-4">
    ...
    <div class="space-y-3">
      ...
      {#if addresses.length < 5}
        <button
          on:click={addAddress}
          class="w-full mobile-btn-secondary"
        >
          + Add Location
        </button>
      {/if}
      ...
      <button
        ...
        class="w-full mobile-btn-primary"
      >
        Find Meeting Point
      </button>
    </div>
